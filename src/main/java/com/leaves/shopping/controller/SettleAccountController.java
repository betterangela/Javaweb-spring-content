...
public class SettleAccountController {
	...
	public String list(ModelMap map, HttpSession session) throws Exception {
	...
	@ResponseBody
	public Object addSettleAccount(@RequestParam("cid") Integer cid, @RequestParam("num") Integer num,
	...
	public Object editSettleAccount(@RequestParam("id") Integer id, @RequestParam("num") Integer num) throws Exception {
		try {
	...
	@ResponseBody
	public Object buy(@RequestParam(value = "tidList", defaultValue = "") String tidList,
			@RequestParam(value = "cid", defaultValue = "") Integer cid,
			@RequestParam(value = "num", defaultValue = "") Integer num, HttpSession session) throws Exception {
		try {
			// 获取用户ID
			Integer personId = ((Person) session.getAttribute(Session.UserSession)).getId();
			if (!"".equals(tidList)) {
				...
				while (tmp.hasMoreElements()) {
					boolean isThisTrue = settleAccountService.buyFromTemporary(personId,
							Integer.parseInt(tmp.nextToken()));
					if (!isThisTrue) {
						isTrue = false;
					}
				}
